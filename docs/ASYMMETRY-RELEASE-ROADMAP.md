...
3. **Portfolio Intelligence** — Plaid-linked holdings, each scored by the rubric (fed by Radar V2),
   with market-open/close push + ElevenLabs audio briefs.
...
| Existing server modules to reuse | `server/src/plaid.ts` (link/exchange/sync/webhook already built), `entitlements.ts` (Pro stub), `market*.ts`, `macro.ts`, `llm*.ts`, `tools/` (fred, treasury, macro-news) |
...
Phasing: **M0–M3 = rubric scorecard surface over Plaid holdings** (the TestFlight beta cutline).
...
- Rebase `supabase/migrations/0001_init.sql` to the Plaid + rubric data model (see Schema below).
...
### M2 — Plaid holdings ingest (read-only)
Extend the existing `server/src/plaid.ts` (link/exchange/sync already built).
- Add `/investments/holdings/get` → persist to `holdings`. Access token stays server-side
  (service-role only).
- **DoD:** Plaid Link → server token exchange (never on device) → holdings populate.
  **Confirm only the `investments` product scope is requested — no transfer/payment scope.**
...
- **DoD:** a Plaid holding renders a scorecard summing to 100 with correct action tier + deploy,
...
### M6 — Daily briefs
- Market-open (~9:25 ET) + close (~4:05 ET) APNs push: AI brief text + **ElevenLabs** audio,
  lock-screen / CarPlay playable.
- **DoD:** at the open trigger, a brief row has both `text` and a playable `audio_url`; clip plays
  from lock screen; **text still ships if ElevenLabs fails.**
...
Beta does **not** need the full 28-signal depth. Ship **M0–M3** (rubric scorecard over Plaid
...
discovery WL ──┘     one engine, two universes    → Reprice Gap → backtest
                     ...
                     per-holding SCORECARD (0–100 + tier + deploy) + deep-signal panel + Reprice Gap
                                  ▼
                     APNs push + ElevenLabs audio brief (open ~9:25 / close ~4:05 ET)
```
...
- `connected_accounts(... provider 'plaid', plaid_item_id, plaid_access_token [RLS service-role only], institution, status, last_synced_at)`.
...
**RLS:** every user-owned table; `plaid_access_token` service-role only; `rubric_*` and
...
1. **Read-only.** Never places/modifies an order; the LLM never types an order. Plaid `investments` scope only.
...
|---|---|---|---|
| Holdings | Plaid Investments `/investments/holdings/get` | linked | M2 |
...
| Audio narration | ElevenLabs | keyed | M6 |
...
- Plaid `client_id` + `secret` (sandbox→production), `investments` product, webhook URL.
- FMP + Finnhub keys (M5), ElevenLabs key + voice id (M6).
...
- **M2:** link a Plaid sandbox account → confirm `holdings` rows populate; grep server logs to confirm
  token exchange is server-side and only `investments` scope requested.
...
- **M6:** trigger an open brief → row has `text` + playable `audio_url`; kill ElevenLabs → text still ships.
...
6. **M2 re-link note:** Plaid Link scope was narrowed to `["investments"]` only (was `auth/transactions/investments/liabilities`; nothing consumed the others). Existing linked users would need a one-time re-link — non-issue for a fresh beta.
